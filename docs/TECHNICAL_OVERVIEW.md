...
(`README.md`, `docs/ARCHITECTURE.md`, `PLAN.md`) as of 2026-08-18. Every number in this document
...
are ten independent, fragile connections doing identical work against the origin — a dropped
...
| `crates/mcp-relay-engine` | SSE framing, replay ring buffer, BLAKE3 hashing — compiled to WASM | Rust → `wasm32-unknown-unknown` |
...
2. `handleSubscribe` (`apps/gateway/src/routes/subscribe.ts`) checks the bearer token, validates
   `originUrl` against an explicit host allowlist (`isAllowedOrigin` — SSRF-relevant input,
...
recursively key-sorted** JSON encoding of `{originUrl, tool, arguments}` — this is the cache
**index key**, identifying "this exact call" independent of how a particular HTTP client happened
to order its JSON keys. Separately, once a real call succeeds, its raw response body is hashed
again to produce the **content address** (`resultHash`) the bytes are stored under. Two different
requests that happen to produce byte-identical output legitimately share a `resultHash` — that's
content-addressing working as intended, not a collision to worry about.
...
- **Step 2**: caller-agent-2 — a genuinely separate call modeling a different caller process — asks
  ...
  a second time, not an inference from speed alone. Measured result: **~14x faster** (30ms vs.
  412ms).
...
| summarizer-agent (optional) | `summarizerAgent.ts` (281 lines) | Calls the Gemini API to turn each notification into a one-line natural-language summary; only runs if a Gemini key is configured |
...
### Two real bugs found building this layer
...
### summarizer-agent: two stacked bugs, found against a real API key
...
React 19 + Tailwind v4, built with Vite, linted with `oxlint`. 1,689 lines across components,
views, and API clients.

| View | File | Purpose |
|---|---|---|
| Live Fan-out | `LiveFanoutView.tsx` (209 lines) | Connects directly from the browser to the gateway's `/subscribe` WebSocket for each subscriber pane; renders real per-subscriber connection state in a topology diagram, not a simulated one |
| Gap Audit | `GapAudit.tsx` (205 lines) | Polls delivery-log endpoints, renders true totals via the counts endpoint, gap causes, and a recent-events throughput chart explicitly labeled as a trend window rather than an all-time count |
| Agents | `AgentsView.tsx` (169 lines) | Shows what each of the four consumers is actually doing, from the real `agent_log` table |
| Cache Metrics | `CacheMetrics.tsx` (197 lines) | Real hit rate, bytes saved, and latency saved for Capability 2 |
...
| Gateway | `wrangler deploy --dry-run` | 82.2 KiB / 30.6 KiB gzip — well inside the Workers free-tier 3 MiB gzip cap |
| Dashboard | `tsc -b` + `vite build` + `oxlint` | clean |
| Agents | `tsc --noEmit` | clean |
| Agents (live gateway + origin-simulator) | resume-agent, gap-aware-agent, ordering-agent | 3/3 reconnects verified gapless, a real killed-process outage correctly resynced, 0 ordering violations across 67+ live events |
| summarizer-agent (real Gemini API, real free-tier key) | live run | real one-line summaries produced end to end after fixing 2 real bugs |
| cacheSharingAgent (live gateway + origin-simulator) | `npm run demo:cache` | cache hit byte-identical, same `originCallSeq`, ~14x faster (30ms vs. 412ms); unrelated scope correctly missed |
...
| Upstream-residency cost | **Not yet measured** — needs a real deploy; stated as an open item |
...
| LLM integration (optional) | Google Gemini API (`gemini-flash-latest`) | — |
...
| Gateway — lib (`cacheClient`, `cacheKey`, `cacheLog`, `feedKey`, `metricsWriter`, `wasmEngine`) + tests | ~406 |
| Rust engine (`sse_framing`, `ring_buffer`, `hash`, `lib`) | 680 |
| Dashboard (components, views, lib) | 1,689 |
| Agents (4 consumers + cache demo + shared + tests) | 872 |
| Origin simulator | 181 |
| Chaos harness (`eval/harness`) | 646 |

Approximate total across the components above: **~7,200 lines**, spanning 4 languages
...
- **Not yet deployed** to a real Cloudflare account. Verified locally against the real `workerd`
  runtime (`vitest-pool-workers`) and real killed/restarted processes; `wrangler deploy --dry-run`
  confirms the bundle and bindings, but a live deploy — and the real GB-seconds measurement of
  upstream-residency cost that requires — hasn't happened yet.
